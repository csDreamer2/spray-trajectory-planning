...
public class QtCommunication : MonoBehaviour
{
    ...
    void HandleWorkpieceData(QtMessage message)
    {
        ...
        try
        {
            ...
            {
                Debug.Log($"📊 接收到点云数据: {pointCloudData.fileName}, 点数: {pointCloudData.pointCount}");
            ...
            {
                Debug.LogWarning("⚠️ 点云数据格式无效");
                StartCoroutine(SimulateWorkpieceLoading("未知工件"));
            }
        }
        catch (System.Exception e)
        {
            Debug.LogError($"❌ 点云数据处理失败: {e.Message}");
            StartCoroutine(SimulateWorkpieceLoading("错误"));
        }
    }
    ...
    IEnumerator SimulateWorkpieceLoading(string fileName = "工件")
    {
        ...
        SendQtMessage("workpiece_loaded", new {
            success = true,
            message = $"Unity成功加载点云: {fileName}",
            fileName = fileName
        });
    ...
    IEnumerator SimulateTrajectoryDisplay()
    {
        ...
        SendQtMessage("trajectory_displayed", new {
            success = true,
            message = "Unity轨迹显示成功"
        });
