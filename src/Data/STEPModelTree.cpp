...
void STEPModelTree::onItemChanged(QStandardItem* item)
{
    ...
    // 查找shared_ptr
    auto it = std::find_if(m_labelToNode.begin(), m_labelToNode.end(),
        [nodePtr](const auto& pair) { return pair.second.get() == nodePtr; });
    
    if (it != m_labelToNode.end()) {
        bool visible = (item->checkState() == Qt::Checked);
        setNodeVisibility(it->second, visible, false);
    }
}
...
std::shared_ptr<STEPTreeNode> STEPModelTreeWidget::getNodeFromItem(QStandardItem* item) const
{
    ...
    auto& labelToNode = m_modelTree->m_labelToNode;
    auto it = std::find_if(labelToNode.begin(), labelToNode.end(),
        [nodePtr](const auto& pair) { return pair.second.get() == nodePtr; });
    
    return (it != labelToNode.end()) ? it->second : nullptr;
}
